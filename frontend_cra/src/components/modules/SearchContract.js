...
import { GetContractDetails,checkPermission } from '../../services/api';
...
export default function SearchContract() {
	...
	useEffect(() => {
	  async function fetch() {
		  const r=await checkPermission('can_query_contract');
		  if(r){
			  setHavePermission(r);
		  }
	  }
    ...
    return (
		...
		{contractDetails && (
		    <div style={{ marginTop: 20 }}>
				...
				<p>开始日期: {contractDetails.contract.start_date}</p>
				<p>结束日期: {contractDetails.contract.end_date}</p>
