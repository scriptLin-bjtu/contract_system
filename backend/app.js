...
app.post('/api/register', async (req, res) => {
    ...
    db.query(queryCheckUser, [username], async (err, results) => {
        ...
        db.query(queryInsertUser, [username, hashedPassword, role], (err, results) => {
            if (err) return res.status(500).json({ code: 500, message: '服务器错误' });
...
// 插入合同信息到合同表
	    const queryInsertContract = 'INSERT INTO contracts (title, description, client_name, created_by, attachment) VALUES (?, ?, ?, ?, ?)';
	    db.query(queryInsertContract, [title, description, client_name, created_by, attachment], (err, result) => {
	        if (err) {
...
app.get('/api/getPendingContracts', authenticateJWT, (req, res) => {
    ...
    const query = `
        SELECT c.id, c.title, c.description, c.client_name, c.created_at 
        FROM contracts c
